 ...
 *
 * Scraps Operas by their wikipedia url and returns a list with the roles as List of strings.
 *
 */
public class DataScrapper {
	...
	public static void main(String[] args) throws IOException {
		List<List<String>> operaList = extractRoles("https://fr.wikipedia.org/wiki/I_due_Foscari");
//		List<List<String>> operaList = extractRoles("https://fr.wikipedia.org/wiki/The_Bear_(op%C3%A9ra)");
		for (List<String> list : operaList) {
	...
	/**
	 * Scraps wikipedia pages based on the opera list containing links to wikipedia.
	 ...
	 * @throws IOException
	 */
	public static List<Opera> extractRoles(List<Opera> operaList) throws FileNotFoundException, IOException {
		for (Opera opera : operaList) {
			try {
				List<List<String>> roles = extractRoles(opera.getUrl());
				opera.setRoles(roles);
		...
		for (Opera opera : operaList) {
			if(opera.getRoles() != null && !opera.getRoles().isEmpty()) {
				if(opera.getHeaderRow().contains("role|Voice type|Premiere Cast|")) {
				} 
			}
		}
		return operaList;
